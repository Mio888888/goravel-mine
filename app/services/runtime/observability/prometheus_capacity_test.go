...
func TestPrometheusMetricsTextIncludesCapacityAndMigrationMetrics(t *testing.T) {
	text := PrometheusMetricsText(MetricsSnapshot{
		CasbinCache:       casbincache.CasbinEnforcerCacheMetrics{Entries: 2, Hits: 3, Misses: 1, Reloads: 1},
		TenantConnections: observabilitycontract.TenantConnectionCapacityMetrics{Pools: 4, RequiredConnections: 480, PostgreSQLBudget: 500, Safe: true},
		MigrationLocks:    observabilitycontract.MigrationLockMetrics{AcquiredTotal: 2, TimeoutTotal: 1, WaitDurationSecondsTotal: 0.5},
	})
