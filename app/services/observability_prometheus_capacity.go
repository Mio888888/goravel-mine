...
func writeQueuePrometheusMetrics(b *strings.Builder, queue QueueBacklogMetric) {
	...
	b.WriteString(fmt.Sprintf("goravel_queue_outbox_events{status=%q} %d\n", QueueOutboxStatusPending, queue.OutboxPending))
	b.WriteString(fmt.Sprintf("goravel_queue_outbox_events{status=%q} %d\n", QueueOutboxStatusProcessing, queue.OutboxProcessing))
	b.WriteString(fmt.Sprintf("goravel_queue_outbox_events{status=%q} %d\n", QueueOutboxStatusFailed, queue.OutboxFailed))
