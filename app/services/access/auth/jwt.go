...
const (
	TenantTokenSubject   = "user"
	PlatformTokenSubject = "platform"
)
...
func ParseApplicationToken(authorization string, requirements TokenRequirements) (TokenInfo, error) {
	tokenText := BearerToken(authorization)
	if tokenText == "" {
		return TokenInfo{}, authcontract.ErrUnauthorized
	}
	...
	token, err := jwt.ParseWithClaims(tokenText, claims, func(token *jwt.Token) (any, error) {
		if token.Method != jwt.SigningMethodHS256 {
			return nil, authcontract.ErrUnauthorized
		}
...
func JWTSecret() (string, error) {
	...
	return "", authcontract.ErrJWTSecretMissing
}
