...
func EvalCondition(condition string, claims map[string]any) bool {
	parser := conditionParser{tokens: tokenizeCondition(condition), claims: claims}
	return parser.parseExpression() && parser.pos == len(parser.tokens)
}

func (p *conditionParser) parseExpression() bool {
	return p.parseOr()
}

func (p *conditionParser) parseOr() bool {
	result := p.parseAnd()
	for p.match("||") {
...
func (p *conditionParser) parseAnd() bool {
...
func (p *conditionParser) parseFactor() bool {
	if p.match("!") {
...
func (p *conditionParser) parseComparison() bool {
	...
	return evalComparison(p.claimValue(leftToken), operator, parseConditionValue(rightToken))
}

func (p *conditionParser) claimValue(token string) any {
...
func evalComparison(left any, operator string, right any) bool {
	switch operator {
	case "==":
