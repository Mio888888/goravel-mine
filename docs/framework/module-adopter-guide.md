...
- `tests/backend/feature/admin/audit_log_test.go`
...
go run . artisan module:admission:check \
  --index=tests/backend/fixtures/moduleadmission/index.valid.json \
...
```bash
MODULE_DISABLED=scheduled-task tests/backend/test.sh ./tests/backend/unit
...
- 验收：`tests/backend/feature/admin/reference_case_test.go`、`MineAdmin-web/tests/e2e/reference-case.spec.ts`。
...
```bash
tests/backend/test.sh ./app/modules ./app/modulecatalog ./app/console/commands ./tests/backend/unit -count=1
tests/backend/test.sh ./tests/backend/feature/admin -run 'TestModuleLifecycleTestSuite|TestModuleLifecycle' -count=1
go run . artisan module:manifest:check --artifacts --frontend
```
