...
- ✅ **模块化架构** - 按功能组织代码，业务能力集中在 modules 中扩展
...
VITE_SECURITY_CSRF=false           # 是否自动注入 CSRF Token
...
- 代理前缀: `/prod`
- CSRF 自动注入默认随生产构建开启；后端关闭 CSRF 时可显式设置 `VITE_SECURITY_CSRF=false`
- 支持生成 sourcemap
...
| **useRouteStore** | `store/modules/useRouteStore.ts` | 动态路由注册/菜单转路由/扁平化 |
