...
function getResourceUrl(url?: string): string | undefined {
  if (!url || /^(?:data:|blob:|https?:\/\/|\/\/)/.test(url)) {
    return url
  }

  const normalizedUrl = url.startsWith('/') ? url : `/${url}`
  const proxyPrefix = import.meta.env.VITE_PROXY_PREFIX?.replace(/\/$/, '')
  if (
    import.meta.env.VITE_OPEN_PROXY === 'true'
      && proxyPrefix
      && !normalizedUrl.startsWith(`${proxyPrefix}/`)
  ) {
    return `${proxyPrefix}${normalizedUrl}`
  }

  const baseUrl = import.meta.env.VITE_APP_API_BASEURL?.replace(/\/$/, '')
...
function getCover(resource: Resource): string | undefined {
  if (resource?.mime_type?.startsWith('image')) {
    return getResourceUrl(resource.url)
  }
  return undefined
}
  ...
  ContextMenu.showContextMenu({
    ...
    items: [
      ...
      },
      {
        ...
        onClick: () => {
          useImageViewer([getResourceUrl(resource?.url) ?? ''])
        },
      },
      {
        label: t('delete'),
