...
import { options } from '~/base/api/dictionary'
...
const useDictStore = defineStore(
  ...
  () => {
    ...
    const load = async (name?: string, replace: boolean = true): Promise<boolean> => {
      const response = await options(name)
      if (name) {
        push(name, response.data as Dictionary[], replace)
        return true
      }
      Object.entries(response.data as Record<string, Dictionary[]>).forEach(([key, value]) => {
        push(key, value, replace)
      })
      return true
    }
