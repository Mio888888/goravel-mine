...
const useMenuStore = defineStore(
  ...
  () => {
    ...
    function init() {
      setTopMenu()
      const { routesRaw } = storeToRefs(routeStore)

      watch(() => routesRaw.value as MineRoute.routeRecord[], (routes: MineRoute.routeRecord[]) => {
        topMenu.value = routes.find((route: any): boolean => route.path === '/')?.children as MineRoute.routeRecord[] ?? []
      }, { immediate: true, deep: true })

      watch((): any => route.name, () => {
        setTopMenu()
