...
import { deleteByIds } from '~/base/api/platformRole.ts'
...
export default function getTableColumns(dialog: UseDialogExpose, formRef: any, t: any): MaProTableColumns[] {
  ...
  return [
    ...
    { type: 'index' },
    // 普通列
    { label: () => t('basePlatformRoleManage.name'), prop: 'name' },
    { label: () => t('basePlatformRoleManage.code'), prop: 'code' },
    ...
    // 操作列
    {
      ...
      operationConfigure: {
        ...
        actions: [
          {
            name: 'setPermission',
            show: ({ row }) => showBtn(['platform:role:getMenu', 'platform:role:setMenu'], row),
            icon: 'material-symbols:checklist-rounded',
            text: () => t('basePlatformRoleManage.setPermission'),
            onClick: ({ row }) => {
              dialog.setTitle(t('basePlatformRoleManage.setPermission'))
          ...
          },
          {
            ...
            show: ({ row }) => showBtn('platform:role:update', row),
            text: () => t('crud.edit'),
            onClick: ({ row }) => {
          ...
          {
            name: 'del',
            show: ({ row }) => showBtn('platform:role:delete', row),
            icon: 'mdi:delete',
            text: () => t('crud.delete'),
          ...
          },
          {
            ...
            show: ({ row }) => row.id === 1 && row.code === 'PlatformSuperAdmin',
            disabled: () => true,
