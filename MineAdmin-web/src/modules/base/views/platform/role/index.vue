...
import { deleteByIds, page } from '~/base/api/platformRole'
...
defineOptions({ name: 'platform:role' })
...
provide('requestSensitiveEvidence', (input: any) => evidenceDialogRef.value!.open(input))
    ...
    if (['add', 'edit'].includes(formType)) {
      ...
      elForm.validate().then(() => {
        ...
        setFormRef.value.saveUserRole().then((res: any) => {
          res.code === ResultCode.SUCCESS ? msg.success(t('basePlatformRoleManage.setPermissionSuccess')) : msg.error(res.message)
          maDialog.close()
        }).catch((err: any) => {
          msg.alertError(err)
        })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('basePlatformRoleManage.mainTitle'),
    subTitle: () => t('basePlatformRoleManage.subTitle'),
  },
  // 表格参数
  tableOptions: {
    on: {
