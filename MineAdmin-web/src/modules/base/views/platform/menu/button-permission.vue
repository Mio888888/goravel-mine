...
useTable('buttonFormTable').then((table: MaTableExpose) => {
  table.setColumns([
    ...
    {
      label: () => t('basePlatformMenuManage.BtnPermission.name'),
      cellRender: ({ row }): any => (
        <el-input v-model={row.title} placeholder={t('basePlatformMenuManage.placeholder.btnName')} />
      ),
    },
    {
      label: () => t('basePlatformMenuManage.BtnPermission.code'),
      cellRender: ({ row }): any => (
        <el-input v-model={row.code} placeholder={t('basePlatformMenuManage.placeholder.btnCode')} />
      ),
    },
    {
      label: () => t('basePlatformMenuManage.BtnPermission.i18n'),
      cellRender: ({ row }): any => (
        <el-input v-model={row.i18n} placeholder={t('basePlatformMenuManage.placeholder.btnI18n')} />
      ),
    },
  ])
})
    ...
    <ma-table ref="buttonFormTable">
      <template #empty>
        <div class="flex items-center justify-center gap-x-2">
          {{ t('basePlatformMenuManage.BtnPermission.noBtn') }}
          <el-button type="primary" plain @click="addItem">
            {{ t('basePlatformMenuManage.BtnPermission.add') }}
          </el-button>
        </div>
      </template>
    </ma-table>
