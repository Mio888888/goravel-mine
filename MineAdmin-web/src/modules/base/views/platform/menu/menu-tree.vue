...
import { deleteByIds } from '~/base/api/platformMenu.ts'
...
<template>
  ...
  >
    <template #default="{ node, data }">
      <div class="mine-tree-node">
        ...
        <div class="do" :class="{ '!inline-block': maTreeRef.elTree.getCurrentKey() === data.id }">
          <el-tag :type="dictColor('menu-type', data.meta?.type) as any" class="mr-2">
            {{ dictLabel('menu-type', data.meta?.type, t) }}
          </el-tag>
          <el-button
            v-if="data.meta?.type === 'M'"
            v-auth="['platform:menu:save']"
          ...
          <el-popconfirm
            ...
            @confirm.stop="async () => {
              if (data.children && data.children.length > 0) {
                msg.notifyError(t('basePlatformMenuManage.deleteChildren'))
                return
              }
              if (data.parent_id !== 0) {
          ...
          >
            <template #reference>
              <el-button
                v-auth="['platform:menu:delete']" size="small" circle type="danger"
              >
    ...
    <template #extra>
      ...
      >
        {{ t('basePlatformMenuManage.addTopMenu') }}
      </el-button>
    </template>
  </ma-tree>
</template>
