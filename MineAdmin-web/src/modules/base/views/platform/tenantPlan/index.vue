...
import { deleteByIds, page } from '~/base/api/platformTenantPlan'
import getSearchItems from './data/getSearchItems.tsx'
import getTableColumns from './data/getTableColumns.tsx'
import useDialog from '@/hooks/useDialog.ts'
import { useMessage } from '@/hooks/useMessage.ts'
import { ResultCode } from '@/utils/ResultCode.ts'
import TenantPlanForm from './form.vue'

defineOptions({ name: 'platform:tenantPlan' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('baseTenantPlanManage.mainTitle'),
    subTitle: () => t('baseTenantPlanManage.subTitle'),
  },
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['platform:tenantPlan:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['platform:tenantPlan:delete']"
          type="danger"
          plain
    ...
    <component :is="maDialog.Dialog">
      <template #default="{ formType, data }">
        <TenantPlanForm v-if="['add', 'edit'].includes(formType)" ref="formRef" :form-type="formType" :data="data" />
      </template>
    </component>
  </div>
</template>
