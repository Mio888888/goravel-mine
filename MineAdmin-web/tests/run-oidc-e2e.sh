...
(
  ...
  go run ./tests/backend/tools/oidc-idp
...
(
  cd "$ROOT_DIR"
  APP_ENV=testing SSO_TEST_ALLOW_LOOPBACK=true go run ./tests/backend/tools/sso-fixture \
...
(
  ...
  yarn playwright test --config=tests/config/playwright.oidc.config.ts
)
