...
func TestRegistryCatalogValidatesMetadataFiles(t *testing.T) {
	registry := NewRegistry([]Module{
		stubModule{
			...
			tests:   []string{"tests/backend/feature/admin/missing_alpha_test.go"},
		},
	})
	...
	for _, expected := range []string{
		...
		"test template does not exist: tests/backend/feature/admin/missing_alpha_test.go",
	} {
		if !strings.Contains(message, expected) {
			t.Fatalf("validation error %q missing %q", message, expected)
		}
...
func TestRegistryAggregatesModuleMetadata(t *testing.T) {
	registry := NewRegistry([]Module{
		stubModule{
			...
			tests:      []string{"tests/backend/feature/admin/alpha_test.go"},
		},
	})
	...
	if got := registry.TestTemplates()[0]; got != "tests/backend/feature/admin/alpha_test.go" {
